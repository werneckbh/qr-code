...
class QRInputItem
{
    ...
    public function encodeModeNum($version)
    {
        try {
            ...
            $val = 0x1;
            $bs->appendNum(4, $val);
            $bs->appendNum(QRSpec::lengthIndicator(QRCode::QR_MODE_NUM, $version), $this->size);
    ...
    public function encodeModeAn($version)
    {
        try {
            ...
            $bs->appendNum(4, 0x02);
            $bs->appendNum(QRSpec::lengthIndicator(QRCode::QR_MODE_AN, $version), $this->size);
    ...
    public function encodeMode8($version)
    {
        try {
            ...
            $bs->appendNum(4, 0x4);
            $bs->appendNum(QRSpec::lengthIndicator(QRCode::QR_MODE_8, $version), $this->size);
    ...
    public function encodeModeKanji($version)
    {
        try {
            ...
            $bs->appendNum(4, 0x8);
            $bs->appendNum(QRSpec::lengthIndicator(QRCode::QR_MODE_KANJI, $version), (int) ($this->size / 2));
    ...
    public function encodeBitStream($version)
    {
        try {
            ...
            if ($this->size > $words) {
                ...
                switch ($this->mode) {
                    case QRCode::QR_MODE_NUM:
                        $response = $this->encodeModeNum($version);
                        break;
                    case QRCode::QR_MODE_AN:
                        $response = $this->encodeModeAn($version);
                        break;
                    case QRCode::QR_MODE_8:
                        $response = $this->encodeMode8($version);
                        break;
                    case QRCode::QR_MODE_KANJI:
                        $response = $this->encodeModeKanji($version);
                        break;
                    case QRCode::QR_MODE_STRUCTURE:
                        $response = $this->encodeModeStructure();
                        break;
